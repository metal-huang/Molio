...
- 形态：阿里云函数计算 FC（Web 函数/Custom Runtime，HTTP server 原样部署，监听 `CAPort`）+ PolarDB Serverless。**单函数 prod**（cn-shenzhen，函数名 `user`；同地域另有支付函数 `wxpay` = pay.molio.cn），自定义域名 `auth.molio.cn` 直连 LATEST 版本（无版本/别名）。

...
| 部署 | **阿里云函数计算 FC**：Web 函数 / Custom Runtime 形态，HTTP server 原样部署（监听 `CAPort`）。第一期手动发布：`node apps/cloud/scripts/deploy-package.mjs` 产出代码包（内含 linux-x64 Node 运行时，启动命令 `/code/runtime/node /code/dist/src/index.js`），控制台上传 ZIP；单函数（prod，**cn-shenzhen**，函数名 `user`），自定义域名 auth.molio.cn 直连 LATEST。⚠️ 常驻实例不感知配置变更，**只改环境变量不生效，须上传新代码包强制实例换新**（2026-08-24 实测）。VPC 连 PG 有冷启动（百 ms~1-2s），登录低频可接受 |
